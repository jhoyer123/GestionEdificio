    ...
    for (const dep of departamentos) {
      ...
      const conceptosParaFactura = conceptos.filter((c) => {
        if (c.frecuencia === "mensual") {
          ...
          const mesFactura = new Date(ultimaFactura.fechaEmision).getMonth();
          return mesFactura < currentMonth;
        }
      ...
      // verificar si existen conceptos para facturar
      if (conceptosParaFactura.length === 0) {
        await t.rollback();
        return res.status(400).json({
          message: `Las facturas con los conceptos actuales ya han sido generadas para este mes/año incluyendo los conceptos unicos.`,
        });
      }
